...
object Ejemplo23GraphXVuelos {
  def main(args: Array[String]): Unit = {
    ...
    println("Ejemplos de vértices: "+ graph.vertices.take(2))
    // Number of routes
    val numroutes = graph.numEdges
    println("Numero de rutas: "+ numroutes)
    println("Ejemplos de rutas: "+ graph.edges.take(2))
    // filtramos los que tengan una distancia superior a 1000
    val mayoresmil=graph.edges.filter { case (Edge(org_id, dest_id, distance)) => distance > 1000 }.take(3)
    println(mayoresmil)
